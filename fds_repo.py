...
#simple chart which shows the effectiveness of different pokemon attack types
TYPE_CHART = {
...
def get_type_advantage(p1_types, p2_types):
    ...
    if not p1_types or not p2_types:
        """if either player has no types, consider the result neutral"""
        return 1.0
    scores = []
    for t1 in p1_types:
        for t2 in p2_types:
...
def create_features(data: list[dict]) -> pd.DataFrame:
    """build a dataFrame from a list of battle records"""
    feature_list = [] #storing features
    for battle in tqdm(data, desc="Extracting features"):
        f = {}
        f["battle_id"] = battle.get("battle_id")
        f["player_won"] = battle.get("player_won", 0) #default = 0 if missing

        # --- Player 1 team ---
        p1_team = battle.get("p1_team_details", [])
        if p1_team:
            """average stats for player no.1"""
        ...
        # --- Player 2 lead ---
        p2_lead = battle.get("p2_lead_details", {}) #info for player no2 first pokemon
        #basic stats for player no2 lead (0 if missed)
        ...
        feature_list.append(f) #add feature dict for the battle to list

    return pd.DataFrame(feature_list).fillna(0) #convert list of dict to dataFrame and replace NaNs with 0

BASIC_STATS = ["hp","atk","def","spa","spd","spe"] #basic stats used in several feture builder

def build_features(battles): #create a feature table from battle data
    rows = [] #store feature dicts for each battle

    for battle in battles:
        features = {} 

        me_team = battle.get("p1_team_details", []) or [] #player no1 stats
        if me_team: # average, total, and max of basic stats
            ...
            #Count team Pokemon with high stats
        ...
        else: #no team info :default value
            for stat in BASIC_STATS:
        ...
        #opponent lead player no2 base stats
        opponent_lead = battle.get("p2_lead_details", {}) or {}
        for stat in BASIC_STATS:
          #extract player no2 lead base stat, default 0  
            features[f"opponent_{stat}"] = float(opponent_lead.get(f"base_{stat}", 0))
            
        for stat in BASIC_STATS: #Stat difference between my team average and opponent lead
        ...
        #Timeline-based featuresT- use first 30 turns
        # initializee damage and counts
        ...
        for event in timeline: # iterate through turns in the timeline
            # Pokemon state (HP% and ...) for both players
            ...
            #count opponent attack vs status moves
            if opponent_move:
    ...
    # convert to DataFrame and replace NaNs with 0.0
...
# basic stats used for feature extraction
BASIC_STATS = ["hp","atk","def","spa","spd","spe"]

#list of Pokeemon types used in type-related features
TYPES = [
    'dragon','electric','fire','flying','ghost','grass',
    'ground','ice','normal','notype','poison','psychic','rock','water']

def get_types(obj):
    """Get Pokemon type list. If missing or invalid, return 'notype."""
    ...
    for t in ts:
        if isinstance(t, str):
            ...
            out.append("notype") # If the type is invalid, use a default value
    return out

def learn_type_effectiveness(battles, min_battles=5):
    """"Use battle results to guess how strong each type is against others"""
    type_win_count  = defaultdict(lambda: defaultdict(int)) # count how many wins each attack_type has against each defend_type
    type_battle_cnt = defaultdict(lambda: defaultdict(int)) # count how many battles happened for each type matchup
    ...
    for b in battles:
        ...
        #Types of the opponent's lead Pokemon  
        p2_types = get_types(p2_lead) or ["notype"]
        
        for p1 in p1_team: # types of a Pokemon on Player no1 team
            p1_types = get_types(p1) or ["notype"]
            for a in p1_types: # a = attacking type (from P1)
                for d in p2_types: # d = defending type (from P2)
                    type_battle_cnt[a][d] += 1
                    # if P1 won the battle, count it as a win for this type matchup
                    if b.get("player_won") == 1:  
                        type_win_count[a][d] += 1

                    # also count the reverse matchup (P2 attacking P1)
                    type_battle_cnt[d][a] += 1
                    if b.get("player_won") == 0:  
                        type_win_count[d][a] += 1
                        
    # initialize effectiveness table with default 1.0 (neutral)
    eff = defaultdict(lambda: defaultdict(lambda: 1.0))
    for a in type_battle_cnt: # For each attacking type
        for d in type_battle_cnt[a]: # For each defending type
            n = type_battle_cnt[a][d]
            if n < min_battles:
                eff[a][d] = 1.0 #not enough data, treat as neutral
            else:
               # how often the attacker wins
...
def type_matchup_score(attacker_types, defender_types, eff_table):
    """Check if the attacker's types are strong or weak against the defender's types.
Returns a score where 1.0 means normal"""
    if not attacker_types or not defender_types:
        return 1.0
    scores = []
    for at in attacker_types:
        
        # Create a list of multipliers for this attacker type vs all defender types.
        mults = [eff_table[at].get(dt, 1.0) for dt in defender_types]
        scores.append(max(mults) if mults else 1.0)
        # Add the highest multiplier for this attacker type
    return float(np.mean(scores)) if scores else 1.0
    
"""Turns the first battle events into simple stats we can use. Useful for building model features"""
def summarize_timeline_30(battle, max_turns=30):
    tl = (battle.get("battle_timeline", []) or [])[:max_turns]
    if not tl: """ # If timeline is empty, return a dict of zeros for all features"""
    ...
    def approx_damage(hplist):
        """شpproximate damage per turn from a list of HP% values.
        We compute only positive drops between consecutive turns"""
    ...
    def last_k_damage(hplist, k=5):
        """approximate average damage per turn in the last k turns"""
        if len(hplist) < 2: return 0.0
        arr = np.array(hplist, dtype=float)
        diffs = np.maximum(0, arr[:-1] - arr[1:])
    ...
    # Initialize sequences and counters
    p1_hp, p2_hp = [], []
    ...
    lead_flags = [] # Truee if P1 is leading at that turn
    p1_lead_first10 = 0 # numbers of turns in first 10 where P1 leads

    prev_p1_hp = prev_p2_hp = None

    for idx, ev in enumerate(tl, start=1):
        # state dicts for both players
        p1s = ev.get("p1_pokemon_state") or {}
        p2s = ev.get("p2_pokemon_state") or {}

        # see if my active Pokemon name changed then switch
        n1 = p1s.get("name")
        if prev_p1_name and n1 and n1 != prev_p1_name:
            p1_switches += 1
        prev_p1_name = n1 or prev_p1_name

        # detect if opponent active Pokemon name changed then switch
        n2 = p2s.get("name")
        ...
        # Count how many status effects each player has (ignore 'nostatus')
        ...
        if p1s and p2s:
            # P1 is leading if P1 HP% > P2 HP%
            lead = (p1s.get("hp_pct", 0) > p2s.get("hp_pct", 0))
            lead_flags.append(lead)
            # Count how many of the first 10 turns P1 leads
            if lead and idx <= 10: p1_lead_first10 += 1
                
    # count lead changes: number of times lead switches between turns
    lead_changes = 0
    if len(lead_flags) >= 2:  # need at least 2 turns to detect a change
        lf = np.array(lead_flags, dtype=int)
        lead_changes = int(np.sum(lf[1:] != lf[:-1])) # Count how many times value flips between turns

   #Build output dictionary of features
    out = {
        "n_turns": len(tl),
...
def build_enhanced_features(battles, TYPE_EFF):
    """Create advanced battle features using team stats, timeline info,
and learned type advantages, then return them as a DataFrame"""
    rows = []
    ...
    for b in battles:
        ...
        if me_team: # aggregate basic stats across team
            for stat in BASIC_STATS:
           ...
           # how many of my Pokemon are fast?
        ...
        else: # If no team data, set all my stats to 0
            for stat in BASIC_STATS:
        ...
        #Opponent lead stats (P2)
        opp_lead = b.get("p2_lead_details", {}) or {}

       # differences between my average stats and opponent's stats
        for stat in BASIC_STATS:
        ...
        #ttimeline summary features (first 30 turns)
        f.update(summarize_timeline_30(b, max_turns=30))

        p2_types = get_types(opp_lead) or ["notype"] # types of opponent lead
        
        # for each of my team Pokemon, evaluate how good its types are vs P2 lead types
        team_type_scores = []
        for p in me_team:
        ...
        # Summary stats of P1 vs P2 lead type matchups
        ...
        # Sum of P1's average stats across HP, ATK, DEF, SPA, SPD, SPE
        p2_total = sum([f.get(f"opponent_{s}",0.0)   for s in BASIC_STATS])
        # Sum of opponent's base stats for the same set
        f["stat_total_diff"]          = p1_total - p2_total

        #damage per move for each side
        ...
        rows.append(f)
    # زonvert to DataFrame and replace NaNs with 0.0
